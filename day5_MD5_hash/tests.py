...
from python import part1 as d1p1
from python import part2 as d1p2

# md5 hashs takes a while in python
class TestDay3(unittest.TestCase):
    def test_part1(self):
        input_string = "abc5017308"
        self.assertEqual(d1p1.hash(input_string), "000008f82")
        assert_keys = [3231929, 5017308, 5278568, 5357525,
                5708769, 6082117, 8036669, 8605828]
        password, test_keys = d1p1.run("abc")
    ...
    def test_part2(self):
        input_string = "abc5017308"
        self.assertEqual(d1p2.hash(input_string), "000008f82")
        assert_keys = [3231929, 5357525, 5708769, 8036669,
        8605828, 8609554, 13666005, 13753421]
        password, test_keys = d1p2.run("abc")
